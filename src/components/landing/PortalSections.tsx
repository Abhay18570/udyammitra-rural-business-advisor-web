...
function Heading({ eyebrow, title, description }: { eyebrow?: string; title: string; description?: string }) {
  const { text } = useUi()
  return <div className="portal-heading">{eyebrow && <span className="portal-kicker">{text(eyebrow)}</span>}<h2>{text(title)}</h2>{description && <p>{text(description)}</p>}</div>
}
