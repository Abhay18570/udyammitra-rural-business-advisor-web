import { loginDestination } from '../../routes/roleRouting'
...
export function LoginPage() {
  ...
  const onSubmit = async (values: LoginFormData) => { setServerError(''); try { const user = await login(values); const destination = loginDestination(user.role, (location.state as { from?: string } | null)?.from); navigate(destination, { replace: true }) } catch (error) { setServerError(getApiErrorMessage(error, 'Unable to sign in. Please try again.')) } }
