...
export function DashboardPage() {
  ...
  if (loading) return <div className="entrepreneur-dashboard"><div className="dashboard-loading" role="status"><LoaderCircle className="dashboard-spinner" aria-hidden="true" /><span><LocalizedText value={"Loading your entrepreneur workspace…"} /></span></div><div className="dashboard-skeleton" aria-hidden="true"><i /><i /><i /><i /></div></div>
  if (error || !profile) return <div className="entrepreneur-dashboard"><section className="dashboard-error" role="alert"><AlertCircle aria-hidden="true" /><h1><LocalizedText value={"We couldn't load your profile."} /></h1><p><LocalizedText value={error || 'Please try again.'} /></p><div><Button type="button" onClick={retry}><LocalizedText value={"Retry"} /></Button><Link className="button button--outline" to="/profile"><LocalizedText value={"Go to Profile"} /></Link></div></section></div>
